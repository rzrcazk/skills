...
1. 基础使用
   ```python
   from templates.character_narration import CharacterNarration, DottyCharacter

   # 创建独白管理器
   narration = CharacterNarration()

   # 添加独白
   narration.add_curiosity(15.0, "这是为什么呢？", "歪头思考")
   narration.add_excitement(45.0, "原来如此！", "跳起来")

   # 生成脚本
   script = narration.generate_script()
   print(script)

   # 保存配置
   narration.save("narration_config.json")
   ```
...
2. 在Manim场景中使用角色
   ...
   class MyScene(Scene):
       def construct(self):
           # 创建角色
           dotty = DottyCharacter()
           character = dotty.create_character(scale=0.8)
           character.to_corner(DR)

           self.play(FadeIn(character))

           # 播放情绪动画
           dotty.animate_emotion("curious", duration=2)

           # 显示思考气泡
           dotty.show_thought_bubble("这是什么原理？", duration=3)
   ```

3. 使用预设场景
   ```python
   from templates.character_narration import NarrationScenarios

   # 获取勾股定理主题的预设独白
   narration = NarrationScenarios.pythagorean_theorem()
   narration.save("narration_pythagorean.json")
   ```

4. 生成TTS音频列表
   ```python
   tts_config = narration.generate_tts_config()
   # 输出到CSV供generate_tts.py使用
   ```
